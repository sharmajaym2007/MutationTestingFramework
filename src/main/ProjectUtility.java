...
public class ProjectUtility {
	...
	public static void createCopy(int mutantNo) throws IOException {
		FileUtils.copyDirectory(new File(projPrefix), new File(projPrefix + SEPARATOR + mutantNo));
	}

	public static void deleteCopy(int mutantNo) throws IOException {
		FileUtils.deleteDirectory(new File(projPrefix + SEPARATOR + mutantNo));
	}
	...
	public static void cleanUp() {
		try {
			File f = new File(projPrefix.replace("/cucumber-reporting", ""));
